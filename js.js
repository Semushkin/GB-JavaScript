...
const map = {
  ...
  render(snakePointsArray, foodPoint, obstaclePoint) {
    ...
    const obstaclePointKey = `x${obstaclePoint.x}_y${obstaclePoint.y}`;
    const obstacleCell = this.cells[obstaclePointKey];
...
const score = {
  ...
  init() {
    this.view = document.getElementById("statistic");
    this.setScore(0);
  },

  setScore(currentScore) {
    this.view.innerHTML = '';
    this.view.innerHTML = `Score: ${currentScore}`;
  }
};
...
const game = {
  ...
  reset() {
    ...
    this.obstacle.setCoordinates(this.getRandomFreeCoordinate());
    this.render();
  ...
  tickHandler() {
    ...
    if (this.food.isOnPoint(this.snake.getNextStepHeadPoint())) {
      this.snake.growUp();
      this.score.setScore(this.snake.getBody().length - 1);
      this.food.setCoordinates(this.getRandomFreeCoordinate());
